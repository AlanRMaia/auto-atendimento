...
describe("", () => {
  ...
  describe("Iniciando os testes para a criação do pedido", () => {
    it("Verificar se o filtro com o campo Numero atendimento está trazendo o atendimento correto", () => {
      ...
      cy.wait(2000);
      cy.get(path.atendimentoPage.numeroAtendimento).type(
        idPrePedidoFinalizado
      );
    ...
    it("Testando o filtro da consulta com o CPF/CNPJ", () => {
      ...
      //     }
      // })
      cy.get(path.atendimentoPage.cpfCnpj).type(transportador.cpfCnpj, {
        force: true,
      });
      cy.get(path.generic.botaoSubmit)
        .click({ force: true })
      ...
      cy.get(".q-table tbody tr").each(($ele, index, list) => {
        if (
          cy
            .wrap($ele)
            .find('td[class="q-td text-center"] ')
            .contains(transportador.cpfCnpj)
        ) {
          ...
          cy.log(
            ...
            cy.wrap($ele).find('td[class="q-td text-center"] ').invoke("text")
          );
        }
      });
      cy.wait("@gridoperacao");
      cy.get(".q-mb-sm > strong", { timeout: 10000 }).contains(
    ...
    it("Testando o filtro da consulta com a Situação AGUARDANDO PAGAMENTO", () => {
      cy.get(path.atendimentoPage.situacao)
        ...
        .contains(situacao.AGUARDANDOPAGAMENTO)
        .click({ force: true });
      cy.get(path.generic.botaoSubmit)
        .click({ force: true })
        .wait(2000)
      ...
      cy.get(".q-table tbody tr").each(($ele, index, list) => {
        cy.wrap($ele)
          .find('td[class="q-td text-center"] > span ')
          .contains(situacao.AGUARDANDOPAGAMENTO);
      });
    });
    ...
    it("Testando o filtro da consulta com a Situação EM CADASTRAMENTO", () => {
      cy.get(path.atendimentoPage.situacao)
        .click({ force: true })
        .get(path.generic.listaVirtual)
        .contains(situacao.EMCADASTRAMENTO)
        .click({ force: true });
      cy.get(path.generic.botaoSubmit)
        .click({ force: true })
        .wait(2000)
    ...
    it("Testando o filtro da consulta com a Situação PAGAMENTO EFETUADO", () => {
      ...
      cy.get(path.atendimentoPage.situacao)
        .click({ force: true })
        .get(path.generic.listaVirtual)
        .contains(situacao.PAGAMENTOEFETUADO)
        .click({ force: true });
      cy.get(path.generic.botaoSubmit)
        .click({ force: true })
        .wait(2000)
    ...
    it("Testando o filtro da consulta com a Situação PEDIDO FINALIZADO", () => {
      cy.get(path.atendimentoPage.situacao)
        .click({ force: true })
        .get(path.generic.listaVirtual)
        .contains(situacao.PEDIDOFINALIZADO)
        .click({ force: true });
      cy.get(path.generic.botaoSubmit)
        .click({ force: true })
        .wait(2000)
    ...
    it.skip("Testando o filtro da consulta com a Situação PEDIDO REJEITADO", () => {
      cy.get(path.atendimentoPage.situacao)
        .click({ force: true })
        .get(path.generic.listaVirtual)
        .contains(situacao.PEDIDOREJEITADO)
        .click({ force: true });
    ...
    it("Criar pedido e consultar se ele aparece em primeiro na lista", () => {
      ...
      describe("Consultando pelo filtro se o pedido aparece na lista", () => {
        cy.get(path.generic.botaoVoltar)
          .click({ force: true })
          .wait(2000)
          .wait("@listapedidos");
        cy.get(".q-table tbody tr").each(($ele, index, list) => {
          if (index == 0) {
            cy.wrap($ele)
              .find('td[class="q-td text-left"]')
              .contains(idPrePedido);
            cy.wrap($ele)
              .find('td[class="q-td text-center"]')
              .contains(transportador.cpfCnpj);
            cy.wrap($ele)
            ...
            cy.wrap($ele)
              .find('td[class="q-td text-left"]')
              .should("not.have.text", idPrePedido);
          }
        });
      });
